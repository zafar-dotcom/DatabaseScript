...
--select * from trn_module m where m.trn_id =386; 
--update trn_module m set m.module_code = 46 where m.trn_id =386;
--select * from role_detail rd where rd.trn_id=386; 
 -- 46 for cashout and for admin 1
...
--====================================================================================================
declare
  v_trn_id           trn_code.trn_id%type := 408;
  v_description      varchar2(100) := 'Create Fee types';
  v_trn_short_desc   varchar2(100) := 'Create Fee types';
  v_menu_label       varchar2(100) := 'Create Fee types';
  v_menu_url         varchar2(100) := '/admin/Create Fee types';
  v_priority_level   trn_code.priority_level%type := 200;
  v_trn_module_code  trn_module.module_code%type := 1; -- ADMIN
begin
  ...
  values (47, v_trn_id ); --misc module 47
...
--=====================================================================
  ...
  SELECT ROWNUM + (SELECT MAX(RD.ROW_ID) FROM ROLE_DETAIL RD),
   ...
   -- AND E.ENTITY_ID = 2 
  AND E.ENTITY_CODE IN ('INTERPAY')--,'BLUEPENGUIN')
   GROUP BY U.ROLE_ID) R,
   TRN_CODE TC, OPERATION OP 
   WHERE TC.TRN_ID IN (v_trn_id) AND OP.OPR_ID IN(1,2);
end;

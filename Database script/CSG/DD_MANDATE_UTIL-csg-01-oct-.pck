CREATE OR REPLACE PACKAGE DD_MANDATE_UTIL AS
  ...
  ------------------------------------------------------------
  ----------------DD_MANDATE_CANCEL_REQ_V2-----------------------
  ------------------------------------------------------------

PROCEDURE DD_MANDATE_CANCEL_REQ_V2(P_MANDATE_REQ_ID         IN NUMBER,
                                  P_CONFIG_ID              IN NUMBER,
                                  P_SYSTEM_MANDATE_ID       IN NUMBER,
                                  P_CANCELLATION_DATE      IN DATE,
                                  P_CANCELLATION_REASON    IN VARCHAR2,
                                  P_CANCELLATION_SOURCE    IN VARCHAR2,
                                  P_STATUS_CODE            IN NUMBER,
...
PROCEDURE CREATE_DD_MANDATE_WITH_PREAPPROVAL(
 ...
 --------------------------------------------------------------------------
 ------------------UPDATE_DD_MANDATE_CALLBACK_STATUS----------------------
 -------------------------------------------------------------------------
   PROCEDURE UPDATE_DD_MANDATE_CALLBACK_STATUS (
                        P_SYSTEM_REFERENCE_ID     IN VARCHAR2,
                        P_GATEWAY_STATUS_MESSAGE  IN VARCHAR2,
                        P_STATUS_CODE             IN NUMBER,
                        P_STATUS_MESSAGE          IN VARCHAR2,
                        P_RESPONSE_DATE           IN DATE,
                        P_OP_LOG                  IN OUT VARCHAR2,
                        P_DD_MANDATE_SYSTEM_ID    OUT NUMBER,
                        P_MERCHANT_ENTITY_ID      OUT NUMBER,
                        P_MERCHANT_REF_ID         OUT VARCHAR2,
                        P_NETWORK_ID              OUT NUMBER,
                        P_AMOUNT                  OUT NUMBER,
                        P_MOBILE                  OUT VARCHAR2,
                        P_RESPONSE_CODE           OUT NUMBER,  -- Added for exception handling
                        P_RESPONSE_DESC           OUT VARCHAR2  -- Added for exception handling
                         );
                         
  --------------------------------------------------------------------------
  ------------------UPDATE_DD_MANDATE_STATUS_SYSTEM_MANDATE-----------------
  --------------------------------------------------------------------------
PROCEDURE UPDATE_DD_MANDATE_STATUS_SYSTEM_MANDATE(P_SYSTEM_REFERENCE_ID    IN VARCHAR2,
                                     P_GATEWAY_REFERENCE_ID   IN VARCHAR2,
                                     P_GATEWAY_STATUS_MESSAGE IN VARCHAR2,
                                     P_STATUS_CODE            IN NUMBER,
                                     P_STATUS_MESSAGE         IN VARCHAR2,
  ...
  --------------------------------------------------------------------------
  ------------------CREATE_DD_RECURRING_PAYMENT_V2-----------------
  --------------------------------------------------------------------------
 
 PROCEDURE CREATE_DD_RECURRING_PAYMENT_V2( -- INPUT PARAMETERS
                                        P_MERCHANT_ENTITY_ID IN NUMBER,
                                        P_SYSTEM_MANDATE_ID  IN NUMBER,
                                        P_AMOUNT             IN NUMBER,
                                        P_PAYMENT_MODE_ID    IN NUMBER,
                                        ...
                                        O_CURSOR         OUT SYS_REFCURSOR);
                                              ...
                                              --------------------------------------------------------------------------
  ------------------GET_DD_RECURRING_PAYMENT_STATUS_V2----------------------
  --------------------------------------------------------------------------
  
  PROCEDURE GET_DD_RECURRING_PAYMENT_STATUS_V2(P_SYSTEM_MANDATE_ID        IN DD_RECURRING_PAYMENT.SYSTEM_MANDATE_ID%TYPE,
                                            P_X_TRANSACTION_REF IN DD_RECURRING_PAYMENT.X_TRANSACTION_REF%TYPE,
                                            P_SYSTEM_TXN_ID     OUT DD_RECURRING_PAYMENT.SYSTEM_TXN_ID%TYPE,
                                            P_STATUS_CODE       OUT DD_RECURRING_PAYMENT.STATUS_CODE%TYPE,
                                            P_STATUS_MESSAGE    OUT DD_RECURRING_PAYMENT.STATUS_MESSAGE%TYPE);
   ...
   --------------------------------------------------------------------------
  ------------------CREATE_DD_MANDATE_WITH_PREAPPROVAL_V2----------------------
  --------------------------------------------------------------------------
  
  PROCEDURE CREATE_DD_MANDATE_WITH_PREAPPROVAL_V2(
                                            P_REQUEST_DATE           IN DATE,
                                            P_PREAPPROVAL_ID         IN VARCHAR2,
                                            P_MERCHANT_ENTITY_ID     IN NUMBER,
                                            P_MERCHANT_REF_ID        IN VARCHAR2,
                                            P_NARRATION              IN VARCHAR2,
                                            P_PLATFORM_SCHEDULING    IN NUMBER,
                                            ...
                                            P_POST                   IN NUMBER,
                                            P_OP_LOG                 IN OUT VARCHAR2,
                                            P_SYSTEM_MANDATE_ID      OUT NUMBER,
                                            P_RESPONSE_CODE          OUT NUMBER,
                                            P_RESPONSE_DESC          OUT VARCHAR,
                                            P_RET_OVA_SERVCEID       OUT VARCHAR2,
                                            P_RET_OVA_SPID           OUT VARCHAR2
...
BEGIN
    ...
    -- Insert into DD_MANDATES
 -- Generate unique SnakeFlow ID only when STATUS_CODE = 1
        V_SYSTEM_MANDATE_ID := GENERATE_SNAKEFLOW_ID();
...
BEGIN
    ...
    IF P_STATUS_CODE = 1 THEN
        ...
        -- Generate unique SnakeFlow ID only when STATUS_CODE = 1
        V_SYSTEM_MANDATE_ID := GENERATE_SNAKEFLOW_ID();
        P_DD_MANDATE_SYSTEM_ID := V_SYSTEM_MANDATE_ID;
        ...
        INSERT INTO DD_MANDATES
            (MANDATE_ID,
             ...
             LOG_ID,
             SYSTEM_MANDATE_ID)
        SELECT SEQ_DD_MANDATE_CONFIG.NEXTVAL,
               SYSDATE,
               REQUEST_ID,
               MERCHANT_ENTITY_ID,
               MERCHANT_REF_ID,
    ...
    ELSE
        -- Just update DD_MANDATE_REQUEST, no SnakeFlow ID
        UPDATE DD_MANDATE_REQUEST
           SET STATUS_CODE            = P_STATUS_CODE,
               STATUS_MESSAGE         = P_STATUS_MESSAGE,
               GATEWAY_STATUS_MESSAGE = P_GATEWAY_STATUS_MESSAGE,
        ...
        P_DD_MANDATE_SYSTEM_ID := NULL;
    END IF;

    -- Always return values from request
    BEGIN
    ...
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            P_MERCHANT_ENTITY_ID := NULL;
            P_MERCHANT_REF_ID := NULL;
            P_NETWORK_ID := NULL;
            P_AMOUNT := NULL;
            P_MOBILE := NULL;
    END;
...
EXCEPTION
      WHEN OTHERS THEN
        ...
        P_MOBILE := NULL;
        P_RESPONSE_CODE := -1;
        P_RESPONSE_DESC := 'ERROR: ' || SQLERRM;
END UPDATE_DD_MANDATE_CALLBACK_STATUS;
...
PROCEDURE UPDATE_DD_MANDATE_STATUS_SYSTEM_MANDATE(P_SYSTEM_REFERENCE_ID    IN VARCHAR2,
  ...
  BEGIN
    ...
    IF P_STATUS_CODE = 1 THEN
      ...
      -- ? INSERT INTO DD_MANDATES AFTER UPDATING DD_MANDATE_REQUEST
 -- Generate unique SnakeFlow ID only when STATUS_CODE = 1
        V_SYSTEM_MANDATE_ID := GENERATE_SNAKEFLOW_ID();
        P_DD_MANDATE_SYSTEM_ID := V_SYSTEM_MANDATE_ID;
    ...
    ELSE
      ...
      P_DD_MANDATE_SYSTEM_ID := -1; -- NO INSERT HAPPENED
    END IF;

  END UPDATE_DD_MANDATE_STATUS_SYSTEM_MANDATE;
    ...
    BEGIN
      SELECT COUNT(*),
        ...
        INTO V_MANDATE_EXISTS,
             ...
             V_OVA_SP_ID
        FROM DD_MANDATES
         WHERE SYSTEM_MANDATE_ID = P_SYSTEM_MANDATE_ID
         AND MERCHANT_ENTITY_ID = P_MERCHANT_ENTITY_ID
         AND STATUS_CODE = 1; -- ASSUMING 1 MEANS ACTIVE/APPROVED MANDATE
  ...
  END CREATE_DD_RECURRING_PAYMENT_V2;
      --------------------------------------------------------------------------
  ------------------GET_DD_RECURRING_PAYMENT_STATUS_V2----------------------
  --------------------------------------------------------------------------
  ...
  EXCEPTION
    ...
    WHEN OTHERS THEN
      ...
      P_STATUS_CODE    := '-100';
      P_STATUS_MESSAGE := SQLERRM;
  END GET_DD_RECURRING_PAYMENT_STATUS_V2;
...
IS
    ...
    V_OVA_SP_ID           VARCHAR2(50) := NULL;
    V_COUNT               NUMBER := 0;
    V_MANDATE_REQ_ID      DD_MANDATES.MANDATE_REQ_ID%TYPE;  
BEGIN
    -- Generate log
    FORMAT_N_GENERATE_LOG(P_OP_LOG, V_OP_LOG_TYPE);

    -- Step 1: Check if PreApproval already exists in DD_MANDATES
    SELECT COUNT(*)
      INTO V_COUNT
      FROM DD_MANDATES
     WHERE GATEWAY_REFERENCE_ID = P_PREAPPROVAL_ID;
    ...
    IF V_COUNT > 0 THEN
        -- Record already exists ? Fetch SYSTEM_MANDATE_ID
        SELECT SYSTEM_MANDATE_ID
          INTO P_SYSTEM_MANDATE_ID
          FROM DD_MANDATES
         WHERE GATEWAY_REFERENCE_ID = P_PREAPPROVAL_ID
           AND ROWNUM = 1;

        P_RESPONSE_CODE := 1;
        P_RESPONSE_DESC := 'PreApproval already exists. Returning existing SYSTEM_MANDATE_ID: ' || P_SYSTEM_MANDATE_ID;
        RETURN;
    END IF;

    -- Step 2: Resolve OVA IDs if not passed
    IF P_OVA_SERVICE_ID IS NULL THEN
        SELECT MN.ENTITY_ID
          INTO V_MNGATEWAY_ENTITY_ID
          FROM MOBILE_NETWORK MN
   ...
   -- Step 3: Insert into DD_MANDATE_REQUEST
    INSERT INTO DD_MANDATE_REQUEST
      (REQUEST_ID,
       REQUEST_DATE,
       MERCHANT_ENTITY_ID,
    ...
    -- Step 4: Insert into DD_MANDATES
    V_SYSTEM_MANDATE_ID := GENERATE_SNAKEFLOW_ID();
    ...
    VALUES
        (SEQ_DD_MANDATE_CONFIG.NEXTVAL,
         ...
         V_OP_LOG_TYPE.LOG_ID,
         V_SYSTEM_MANDATE_ID)
    RETURNING SYSTEM_MANDATE_ID INTO P_SYSTEM_MANDATE_ID;

    -- Step 5: Final response
    P_RESPONSE_CODE := 1;
    P_RESPONSE_DESC := 'Mandate successfully created with SYSTEM_MANDATE_ID: ' || P_SYSTEM_MANDATE_ID;

EXCEPTION
    WHEN OTHERS THEN
        P_RESPONSE_CODE := -1;
        P_RESPONSE_DESC := 'An error occurred: ' || SQLERRM;
END CREATE_DD_MANDATE_WITH_PREAPPROVAL_V2;

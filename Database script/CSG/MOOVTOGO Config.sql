select * from entity_system_config where config_id=1255 ;
select * from REMIT_ENTITY_PROCESS_CONFIG order by 1 desc; 
select * from entity where entity_id=149935;
select * from entity order by 1 desc;
select * from entity_type order by 1 desc;
select * from country where dialing_code=+228;
-- for currency based on currency_symbol 
Select * from Currency where currency_symbol='XOF';
...
values
  (149982,
   ...
   null,
   null,
   to_date('17-01-2025 04:05:11', 'dd-mm-yyyy hh24:mi:ss'),
   1,
   null,
   null,
   null,
...
select * from entity where entity_id=149935;
select * from REMIT_ENTITY_PROCESS_CONFIG order
select * fro 

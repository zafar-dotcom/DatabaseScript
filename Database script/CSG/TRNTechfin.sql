...
select * from operation  ;  --1



declare
  v_trn_id           trn_code.trn_id%type := 2;   --next value to trn_id
  v_description      varchar2(100) := 'payment via 3rd party';
  v_trn_short_desc   varchar2(100) := 'payment via 3rd party';
  v_menu_label       varchar2(100) := 'Payment via 3rd party';
  v_menu_url         varchar2(100) := '/Kyc/PaymentVia3rdparty';
  v_priority_level   trn_code.priority_level%type := 200;
  v_trn_module_code  trn_module.module_code%type := 1;--Admin get this from  module table
begin
  ...
  values (1, v_trn_id ); --Cashout get this from module table

  insert into trn_operations (trn_id, opr_id)
  values (v_trn_id, 1);  ---1 FOR VIEW
  
 insert into trn_operations (trn_id, opr_id)
  values (v_trn_id, 5);  ---5 FOR POST
--=====================================================================
--ROLE_DETAIL
--=====================================================================
  ...
  SELECT ROWNUM + (SELECT MAX(RD.ROW_ID) FROM ROLE_DETAIL RD),
   ...
   AND UPPER(UR.ROLE_DESCRIPTION) IN ('ADMINISTRATOR')--,'ADMINISTRATOR')
   -- AND E.ENTITY_ID = 2 
  AND E.ENTITY_CODE IN ('Techfin')--,'BLUEPENGUIN')
   GROUP BY U.ROLE_ID) R,
   TRN_CODE TC, OPERATION OP 
   WHERE TC.TRN_ID IN (v_trn_id) AND OP.OPR_ID IN(1,5);
end;

CREATE OR REPLACE PACKAGE BULK_SETTLEMENT_UTIL_EXT IS
  ...
  --======================================================================================================
  --                       PROCEDURES -- ADD_SETTLEMENT_UPDATED_REPUSH_INSTRUC_REC ---
  --======================================================================================================
  PROCEDURE ADD_SETTLEMENT_UPDATED_REPUSH_INSTRUC_REC( --P_RECORD_ID MERCHANT_SETTLEMNT_INSTRUC.RECORD_ID%TYPE,                                             
 ...
 --======================================================================================================
  ...
  --======================================================================================================
  --                       PROCEDURES -- GNRT_SETTLEMENT_REPUSH_INSTRUC ---
  --======================================================================================================   

PROCEDURE GNRT_SETTLEMENT_REPUSH_INSTRUC(P_TRANS_ID      IN VARCHAR2,
...
CREATE OR REPLACE PACKAGE BODY BULK_SETTLEMENT_UTIL_EXT IS
  ...
  BEGIN
    ...
    UPDATE MERCHANT_SETTLEMNT_INSTRUC
        SET 
            REPUSH_TYPE = P_REPUSH_TYPE,
            IS_PUSHED = P_IS_PUSHED,
            IS_PROCESSED = P_IS_PROCESSED,
            IS_REPUSHED = P_IS_REPUSHED
        WHERE TRANS_ID = P_TRNAS_ID;
  ...
  END ADD_SETTLEMENT_UPDATED_REPUSH_INSTRUC_REC;
...
PROCEDURE ADD_SETTLEMENT_UPDATED_INSTRUC_REC( --P_RECORD_ID MERCHANT_SETTLEMNT_INSTRUC.RECORD_ID%TYPE,
  ...
  BEGIN
    OPEN P_RECORD_LIST FOR
      SELECT TRANS_DATE, SUM(AMOUNT) AS AMOUNT, NEW_BANK_NAME, REPUSH_TYPE ,BATCH_NO
        FROM MERCHANT_SETTLEMNT_INSTRUC
       WHERE IS_PUSHED = 1
            AND TRUNC(TRANS_DATE) = TRUNC(P_TRANS_DATE)
         AND IS_REPUSHED = 0
        -- AND IS_PROCESSED IN (0)
       GROUP BY TRANS_DATE, NEW_BANK_NAME, REPUSH_TYPE , BATCH_NO
       ORDER BY 1 DESC;
        ...
        FROM (SELECT A.*, ROWNUM R__
                FROM (SELECT BR.TRANSACTION_DATE as Trans_date,
                        ...
                        FROM BANK_REGISTER BR,
                             ...
                             BANK_ACCOUNT  CBA,
                             BANK_BRANCH   CBB,
                             MERCHANT_SETTLEMNT_INSTRUC MSI,
                             GIP_BANKS_ROUTING_CODES GPB
                       WHERE BR.BANK_ACCOUNT_ID = BA.BANK_ACCOUNT_ID
                         AND BR.CONTRA_BANK_ACCOUNT_ID = CBA.BANK_ACCOUNT_ID
                         AND CBA.BANK_BRANCH_ID = CBB.BANK_BRANCH_ID
                         ...
                         AND CBB.BANK_ID = GPB.BANK_ID(+)
                         AND BR.TRANSACTION_ID = MSI.TRANS_ID(+)
                         AND BR.TRANSACTION_ID IN (SELECT MS.TRANS_ID FROM MERCHANT_SETTLEMNT_INSTRUC MS)
...
PROCEDURE GNRT_SETTLEMENT_REPUSH_INSTRUC(P_TRANS_ID   IN VARCHAR2,
  ...
  BEGIN  
    -- UPDATE BANK_REGISTER_BATCH_DETAIL
    UPDATE BANK_REGISTER_BATCH_DETAIL DD
       SET DD.STATUS      = 3,
           DD.STATUS_DESC = 'CLOSING FOR NEW REPUSH INSTRUC' || '-' ||
                            DD.STATUS_DESC
     WHERE DD.BANK_REGISTER_TRANS_ID IN
           (SELECT REGEXP_SUBSTR(P_TRANS_ID, '[^,]+', 1, LEVEL)
              FROM DUAL
            CONNECT BY REGEXP_SUBSTR(P_TRANS_ID, '[^,]+', 1, LEVEL) IS NOT NULL)
       AND DD.STATUS = 0;
    ...
    DECLARE
      ...
      V_GENERAL_XFR_ENTITY_ID        ENTITY.ENTITY_ID%TYPE := 87559;
      V_GENERAL_XFR_WALLET_ID        ENTITY.ENTITY_ID%TYPE := 177582;
      V_BANK_ACCOUNT_ID              BANK_ACCOUNT.BANK_ACCOUNT_ID%TYPE := NULL;
      V_IPAY_DEFSETTL_BANK_ENTITY_ID ENTITY.ENTITY_ID%TYPE := SYSTEM_PROCESS_UTIL.GET_ENTITY_CONFIG_VALUE('Access Bank',
                                                                                                          'EntityIds'); --ACCESSBANK
    BEGIN
    
      V_BANK_ACCOUNT_ID := SYSTEM_PROCESS_UTIL.GET_POOL_BANK_ACCOUNT(P_BANK_ENTITY_ID     => V_IPAY_DEFSETTL_BANK_ENTITY_ID,
                                                                     P_MERCHANT_ENTITY_ID => 1, --INTERPAY
                                                                     P_POOL_ACCT_NATURE   => 'T',
                                                                     P_CURRENCY_ID        => 1);
      FOR I_INDEX IN (SELECT *
                        FROM BANK_REGISTER BR
                       WHERE BR.TRANSACTION_ID IN
        ...
        -- UPDATE BANK_REGISTER
        UPDATE BANK_REGISTER BR
           SET BR.BANK_ACCT_ACTSTAT_ID          = 3,
               BR.POST                          = 1,
               BR.VALUE_DATE                    = V_SYSTEM_DATE,
               BR.POST_XFR_RESPONSE_DATE        = V_SYSTEM_DATE,
               BR.POST_XFR_RESPONSE_STATUS_CODE = 3,
               BR.POST_XFR_RESPONSE_STATUS_DESC = 'MANUALLY MARKED CLOSING FOR NEW REPUSH INSTRUC ID - ' ||
                                                  V_BANK_REG_TRANS_ID
         WHERE BR.TRANSACTION_ID = I_INDEX.TRANSACTION_ID;
        ...
        INSERT INTO BANK_REGISTER
          (TRANSACTION_ID,
           ...
           LOG_ID,
           VALUE_DATE,
           ENTITY_FEE_REG_TRANS_ID,
           CONTRA_BANK_ACCOUNT_ID,
           BANK_TELLER_ID,
           BANK_BRANCH_CODE,
           PAYEE_NAME,
           PAYEE_PHONE,
           ...
           WALLET_ID,
           ENTITY_RECPT_REG_TRANS_ID,
           POST_XFR_RESPONSE_DATE,
           WALLET_TOPUP_REQ_TRANS_ID,
           INTERBANK_XFRREQ_TRANS_ID,
           INTERWALLET_XFRREQ_TRANS_ID,
           GENERAL_PAYREQ_TRANSID,
           WALLET_ALIASE_ID,
           DDTS_DET_TRANS_ID,
           OTC_TXNREQ_TRANS_ID,
           REMITTANCE_REG_TRANSID,
           RECONCILED,
           BANK_ACC_RECON_ID,
           POST_TO_BANK_TXNORIGIN_ID,
           POST_XFR_RESPONSE_STATUS_CODE,
           POST_XFR_RESPONSE_STATUS_DESC,
           SYSTEM_TRANS_ID,
           IS_APPROVED_FOR_SETTLEMENT,
           CLIENT_TIMESTAMP,
           BANK_VOUCHER_MF_ID,
           USER_ID_AUTHORIZER,
           BANK_ACCT_ACTSTAT_ID,
           PAYTOMERCH_FOR_BANKREG_TXNID,
           BULK_SETTLEMENT_BATCH_NUMBER,
           BULK_SETTLEMENT_BATCH_DATE)
        VALUES
          (V_BANK_REG_TRANS_ID,
           V_SYSTEM_DATE,
           NULL,
           V_BANK_ACCOUNT_ID,
           14,
           V_GENERAL_XFR_ENTITY_ID,
           -1 * ABS(I_INDEX.AMOUNT),
           I_INDEX.NARRATION, --P_NARRATION,
           1,
           NULL,
           NULL,
           0,
           ...
           NULL,
           NULL,
           I_INDEX.CONTRA_BANK_ACCOUNT_ID,
           NULL,
           NULL,
           NULL,
           NULL,
           ...
           V_SYSTEM_DATE,
           NULL,
           I_INDEX.TRANSACTION_ID,
           V_GENERAL_XFR_WALLET_ID,
           NULL,
           NULL,
           NULL,
           NULL,
           ...
           2,
           NULL,
           I_INDEX.BULK_SETTLEMENT_BATCH_NUMBER,
           V_SYSTEM_DATE
           );
        ...
        UPDATE MERCHANT_SETTLEMNT_INSTRUC MSI
           SET MSI.TRANS_ID_NEW   = V_BANK_REG_TRANS_ID,
               ...
               MSI.IS_REPUSHED    = 1, 
               MSI.STATUS_CODE    = 4,
               MSI.STATUS_MESSAGE = 'MANUALLY MARKED CLOSING FOR NEW REPUSH INSTRUC ID - ' ||
                                    V_BANK_REG_TRANS_ID
         WHERE MSI.TRANS_ID = I_INDEX.TRANSACTION_ID;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      P_STATUS_DESC := 'Error: ' || SQLERRM;
    END;
  END GNRT_SETTLEMENT_REPUSH_INSTRUC;

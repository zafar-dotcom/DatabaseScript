...
CREATE OR REPLACE PACKAGE BODY DD_MANDATE_UTIL AS
  ...
  PROCEDURE CREATE_DD_MANDATE_REQUEST(P_REQUEST_DATE           IN DATE,
    ...
    V_FEE_TYPE_ID           NUMBER(12):=NULL;
    V_ROW_MANDATE           DD_MANDATES%ROWTYPE;
    V_APPROVED_NEW_MANDATE  NUMBER(12):=NULL;
    V_APPROVED_NEW_MANDATE_REQ_ID NUMBER(12):=NULL;
  BEGIN
    -- GENERATE LOG
    FORMAT_N_GENERATE_LOG(P_OP_LOG, V_OP_LOG_TYPE);
    
    V_FEE_TYPE_ID :=SYSTEM_PROCESS_UTIL.GET_FEE_TYPE_ID(P_MERCHANT_ENTITY_ID,P_FEE_TYPE_NAME);
    
    IF V_FEE_TYPE_ID IS NULL 
      THEN
        P_RESPONSE_CODE := 0;
        P_RESPONSE_DESC := 'Invalid FeeTypeCode' ||
                         P_FEE_TYPE_NAME;
      RETURN;
     END IF;
    ...
    BEGIN
      SELECT MANDATE_ID
       ...
       WHERE MERCHANT_ENTITY_ID = P_MERCHANT_ENTITY_ID
         AND MOBILE = P_MOBILE
         AND FEE_TYPE_NAME = P_FEE_TYPE_NAME
         AND STATUS_CODE = 1
    ...
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        -- No mandate for merchant, check for mobile+fee type only
      BEGIN
        SELECT *
          INTO V_ROW_MANDATE
          FROM DD_MANDATES
        ...
        V_APPROVED_NEW_MANDATE :=SEQ_DD_MANDATE_CONFIG.NEXTVAL;
        P_EXISTING_REQUEST_ID :=V_APPROVED_NEW_MANDATE;
        V_APPROVED_NEW_MANDATE_REQ_ID :=SEQ_DD_MANDATE_REQUEST.NEXTVAL;  -- New request id;
       ...
       VALUES 
           (
             V_APPROVED_NEW_MANDATE,        -- New mandate id
             V_ROW_MANDATE.DD_CONFIG_DATE,
             V_APPROVED_NEW_MANDATE_REQ_ID,
             P_MERCHANT_ENTITY_ID,           -- Replace with new merchant
             P_MERCHANT_REF_ID,                --Replaced with new merchant ref
             V_ROW_MANDATE.MOBILE,
             V_ROW_MANDATE.NETWORK_ID,
             V_ROW_MANDATE.NARRATION,
             V_ROW_MANDATE.CURRENCY_ID,
             V_ROW_MANDATE.FREQUENCY_ID,
             V_ROW_MANDATE.NO_OF_INSTALLMENTS,
             V_ROW_MANDATE.START_DATE,
             V_ROW_MANDATE.END_DATE,
             V_ROW_MANDATE.DAY_OF_DEBIT,
             V_ROW_MANDATE.IS_PRE_AUTH_ENABLED,
             V_ROW_MANDATE.PLATFORM_SCHEDULING,
             V_ROW_MANDATE.OVA_SERVICE_ID,
             V_ROW_MANDATE.OVA_SP_ID,
             1,                              -- New active status
             'APPROVED', -- Custom status message
             V_ROW_MANDATE.SYSTEM_REFERENCE_ID,
             V_ROW_MANDATE.GATEWAY_REFERENCE_ID,
             V_ROW_MANDATE.GATEWAY_STATUS_MESSAGE,
             SYSDATE,                        -- New response date
             SYSDATE,                        -- Last update time
             V_FEE_TYPE_ID,
             P_FEE_TYPE_NAME,
             V_ROW_MANDATE.POST,
             V_ROW_MANDATE.LOG_ID
           );
   
        --insert into dd_mandates request table for record to check status endpoint
        
        -- ? Insert into DD_MANDATE_REQUEST from DD_MANDATES
      ...
      SELECT 
         ...
         M.SYSTEM_REFERENCE_ID,
         M.GATEWAY_REFERENCE_ID,
         M.GATEWAY_STATUS_MESSAGE,
      ...
      FROM DD_MANDATES M
      WHERE M.MERCHANT_REF_ID = P_MERCHANT_REF_ID;
       ...
       -- P_EXISTING_REQUEST_ID := NULL;
        ...
        BEGIN
          SELECT REQUEST_ID
            ...
            WHERE MERCHANT_REF_ID = P_MERCHANT_REF_ID 
